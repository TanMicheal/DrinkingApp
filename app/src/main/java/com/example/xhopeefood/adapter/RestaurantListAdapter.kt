...
import com.example.xhopeefood.models.Hours
import com.example.xhopeefood.models.RestaurentModel
import java.util.*

class RestaurantListAdapter(val restaurantList: List<RestaurentModel?>?, val clickListener: RestaurantListClickOnListener): RecyclerView.Adapter<RestaurantListAdapter.MyViewHolder>() {
    ...
    inner class MyViewHolder(view: View): RecyclerView.ViewHolder(view)
    {
        ...
        fun bind(restaurentModel: RestaurentModel?) {
            ...
            tvRestaurantHours.text = "Giờ mở cửa: " + getTodayHours(restaurentModel?.hours!!)
    ...
    private fun getTodayHours(hours: Hours): String? {
        ...
        val day : String = java.text.SimpleDateFormat("EEEE", Locale.ENGLISH).format(date.time)
        return when(day) {
            "Sunday" -> hours.Sunday
            "Monday" -> hours.Monday
    ...
    interface RestaurantListClickOnListener {
